...
For `AskUserQuestion`, use header `Ask route`, set metadata source to
`ringframe.ask`, and put the summary below in the Proceed option's
`preview`. Say the person may type a revision or select another route.
