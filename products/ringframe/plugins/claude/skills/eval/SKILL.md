...
Before the first spawn, read `${CLAUDE_SKILL_DIR}/config.toml` once. Its
`[roles]` table holds this plugin's tiers, `<role> = { model = "…", effort =
"…" }`. A role's model and its effort, each on its own, are the first of: the
invocation's word; that file; nothing, in which case leave it out of the
`Agent` call's `model` and `effort` fields so the harness default applies. If
the file is missing or unreadable, say so once in the report and use only the
invocation's words. Never guess a model or an effort. "A role's values" below
means these resolved values.
...
@.fab7/rf/tmp/eval-<eval_id>-context.md --host claude-code`. It publishes the
